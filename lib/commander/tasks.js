...
exports.getAllTasks = function(callback) {
    ...
    Task.find({}, function(err, tasks) {
        if(err) {
            ...
            log.debug('Got %d task(s)', tasks.length);
            callback(tasks);
        }
    });
};
...
exports.getRunsListByTaskId = function(id, callback) {
    log.debug('Querying runs list for task with id "%s"', id);
    ...
    Run.find({task_id: id})
        ...
        .exec(function(err, runs) {
            if(err) {
                log.error('Error occurred while querying runs list for task "%s": %s', id, err.message);
                callback([]);
            } else {
                log.debug('Found %d runs', runs.length);
                callback(runs);
            }
        });
};
...
exports.clearTaskRunLog = function(id, callback) {
    Run.remove({task_id: id}, function(err, count) {
        if(err) {
            log.error('Error occurred while clearing task "%s" log: %s', id, err.message);
            callback(0);
        } else {
            log.debug("Removed %d record(s)", count);
            callback(count);
        }
    });
};
