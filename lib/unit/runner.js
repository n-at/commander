...
exports.executeScript = function(script, callback) {
    var fileName = path.join(config.get('tmp_path'), uuid.v4());
    ...
    fs.writeFile(fileName, script, function(err) {
        ...
        runShellScriptFromFile(fileName, function(err, result) {
            fs.unlink(fileName, function(err) { //remove temporary script
                if(err) {
                    logger.info('Cannot remove temporary script "%s": %s', fileName, err.message);
                } else {
...
exports.executePreset = function(presetName, callback) {
    if(!presetName) {
        callback(new Error('Empty preset name'));
        return;
    }
